...
def process(args, id, harm_prompt):
    # FlipAttack
...
if __name__ == "__main__":
    ...
    if args.parallel:
        result_dict = Parallel(n_jobs=-1)(
            delayed(process)(args, id, harm_prompt)
            for id, harm_prompt in tqdm(enumerate(adv_bench["goal"][args.begin:args.end]))
        )
    else:
        raise TypeError("run main.py")
       
    if eval:
        for id, result in enumerate(result_dict):
            all_count += 1
            dict_success_count += result["judge_success_dict"]
            gpt_success_count += result["judge_success_gpt4"]
